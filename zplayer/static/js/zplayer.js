...
var vjs = function (id, options, ready) {
	...
	if (!tag || !tag.nodeName) { // re: nodeName, could be a box div also
		throw new TypeError('The element or ID supplied is not valid. (zplayer)'); // Returns
	}
...
var zplayer = window['zplayer'] = vjs;

// CDN Version. Used to target right flash swf.
vjs.CDN_VERSION = 'undefined';
...
vjs.options = {
	...
	// 'techOrder': ['vimeo', 'youtube', 'flash', 'html5'],
	...
	//'language': document.getElementsByTagName('html')[0].getAttribute('lang') || navigator.languages && navigator.languages[0] || navigator.userLanguage || navigator.language || 'ko',
	'language': 'ko',
...
// The added (+) blocks the replace from changing this undefined string
//if (vjs.CDN_VERSION !== 'GENERATED' + '_CDN_VSN') {
//    zplayer.options['flash']['swf'] = vjs.ACCESS_PROTOCOL + 'vjs.zencdn.net/' + vjs.CDN_VERSION + '/video-js.swf';
//}
...
vjs.setLocalStorage = function (key, value) {
    try {
        ...
        if (e.code == 22 || e.code == 1014) { // Webkit == 22 / Firefox == 1014
            vjs.log('LocalStorage Full (ZPlayer)', e);
        } else {
            if (e.code == 18) {
                vjs.log('LocalStorage not allowed (ZPlayer)', e);
            } else {
                vjs.log('LocalStorage Error (ZPlayer)', e);
            }
        }
    }
};
 ...
 *
 *     // using url string
 *     zplayer.xhr('http://example.com/myfile.vtt', function(error, response, responseBody){});
 *
 *     // or options block
 *     zplayer.xhr({
...
vjs.util.arrayKeyExists = function (key, search) {
...
vjs.util.getProtocol = function (file) {
    return file.split(':').shift().toLowerCase();
};

vjs.util.getExtension = function (file) {
    return file.split('.').pop().toLowerCase();
};

vjs.util.getFileType = function (file) {
    var mimes = {
    ...
    var file_type;
    var file_protocol = vjs.util.getProtocol(file);

    if (file_protocol === 'rtmp') {
        file_type = mimes['rtmp'];
    } else if (file_protocol === 'rtsp') {
        file_type = mimes['rtsp'];
    } else {
        file_type = vjs.util.getExtension(file);

        if (vjs.util.arrayKeyExists(file_type, mimes) === true) {
            file_type = mimes[file_type];
        } else {
            file_type = 'unknown/unkown';
        }
    }
...
//vjs.util.millToTimecode = function (seconds, timeFormat) {
...
vjs.util.setInnerText = function (element, text) {
...
vjs.util.addEventListener = function (element, event, cb) {
...
vjs.Component.prototype.addChild = function(child, options){
    ...
    if (typeof child === 'string') {
        ...
        component = new window['zplayer'][componentClass](this.player_ || this, options);
...
vjs.Slider = vjs.Component.extend({
	...
	init: function (player, options) {
		...
		this.on(player, 'mousemove', this.onProgressMouseMove);
		this.off(player, 'mouseout', this.onProgressMouseMove);
	}
});
...
vjs.Slider.prototype.onMouseMove = function (event) {
	//console.log('slider onMouseMove', event);
};

vjs.Slider.prototype.onProgressMouseMove = function (event) {
	//console.log('slider onProgressMouseMove', event);
};
...
vjs.Slider.prototype.calculateDistancePixel = function (event, w) {
	var el, box, boxX, boxY, boxW, boxH, handle, pageX, pageY;
	...
	if (this.options()['vertical']) {
		...
		//console.log(pageX , boxX, boxW);
		return Math.max(0, Math.min((boxW - bx), (pageX - boxX - (bx/2))));
	}
};
...
// Initialize tech instance
	this.tech = new window['zplayer'][techName](this, techOptions);
	...
	if (typeof source === 'string') {
		...
		if (!window['zplayer'][this.techName]['canPlaySource'](source)) {
			...
			this.ready(function () {
				...
				if (window['zplayer'][this.techName].prototype.hasOwnProperty('setSource')) {
...
// playlist
vjs.Player.prototype.playlist_;
...
vjs.Player.prototype.playrun_ = function () {
	...
	if (c.hasOwnProperty('tracks')) {
		var obj = this;
		c.tracks.forEach(function (track, i) {
			obj.addRemoteTextTrack(track);
		});
	}
...
vjs.Player.prototype.clip = function (source) {
...
vjs.Player.prototype.clip_ = function (sources) {

	var c = {};
	//.clip('https://www.youtube.com/watch?v=7NMhjmLGRak')
	if (typeof sources === 'string') {
		c.source = this.clipSource_([sources]);

	//.clip([
	//		"rtmp://119.205.215.21/live/mp4:liveTV",
	//		'http://119.205.215.18:1935/live/egcTV.stream/playlist.m3u8',
	//		'rtsp://119.205.215.18/live/mp4:egcTV.stream'
	//])
	//.clip([
	...
	//])
	} else if (vjs.obj.isArray(sources)) {
	...
	//	title: 'Youtube',
	//	poster: 'http://119.205.215.21/Upload/20140924195416.jpg',
	//	tracks: [
	//		{kind: 'captions', src: 'captions.ko.vtt', srclang: 'ko', label: 'Korean', default: true},
	...
	//		'rtsp://119.205.215.18/live/mp4:egcTV.stream'
	//	],
	//	title: 'LIVE RTMP/HLS/RTSP (SmartFarmTV)',
	//	poster: 'http://119.205.215.21/Upload/20140924195416.jpg',
	//	tracks: [
	//		{kind: 'captions', src: 'captions.ko.vtt', srclang: 'ko', label: 'Korean', default: true},
	//		{kind: 'captions', src: 'captions.vtt', srclang: 'en', label: 'English'}
	...
	//})
		...
		if (sources.source !== undefined) {
			c.source = this.clipSource_(sources.source);
		}

		//poster
		if (sources.poster) {
			c.title = sources.poster;
		}

		//tracks
		if (sources.tracks) {
			c.tracks = sources.tracks;
		}

		//titles
		if (sources.title) {
			c.title = sources.title;
		}

		//thumbnails
		if (sources.thumbnails) {
			c.thumbnails = sources.thumbnails;
		}
	}
...
vjs.Player.prototype.clipSource_ = function(sources) {

	var obj =this;
	var c = [];

	if (vjs.obj.isArray(sources)) {

		sources.forEach(function (source, i) {
			if (typeof source === 'string') {
				c.push(obj.clipType_(source));
			} else {
				c.push(source);
			}
		});
...
vjs.Player.prototype.clipType_ = function (source) {
	...
	//	//'OGV':'video/ogg'
	//};
	//var file_protocol = source.split(':').shift().toLowerCase();
	//var file_type = source.split('.').pop().toLowerCase();
	//
	//if(file_protocol in formats) {
	//	return {src:source, type:formats[file_protocol]};
	//}
	//
	//if(file_type in formats) {
	//	return {src:source, type:formats[file_type]};
	//}
	//
	//return {src:source, type:'unknown/unknown'};
...
vjs.TimeDivider.prototype.createEl = function () {
	return vjs.Component.prototype.createEl.call(this, 'div', {
		className: 'vjs-time-divider vjs-control',
...
//});
//
//vjs.SeekHolder.prototype.createEl = function () {
//    return vjs.Slider.prototype.createEl.call(this, 'div', {
//        className: 'vjs-progress-padding'
//    });
//};

vjs.SeekHolder = vjs.Component.extend({
	/** @constructor */
	init: function (player, options) {
		vjs.Component.call(this, player, options);
...
/**
 * The SeekTip component contains the seek bar, load progress,
 * and play progress
 *
 * @param {vjs.Player|Object} player
 * @param {Object=} options
...
vjs.SeekTip.prototype.createEl = function () {
	return vjs.Slider.prototype.createEl.call(this, 'div', {
		className: 'vjs-tooltip',
		innerHTML: "<div class='vjs-tooltip-bg'></div><div class='vjs-tooltip-text-wrapper'><span class='vjs-tooltip-text'>" + this.defaultValue + "</span></div>"
	});
};

vjs.SeekTip.prototype.onProgressMouseMove = function (e) {
	this.el().style.left = this.position_(e) + 'px';
	this.el_.children[1].children[0].innerHTML = this.mousetime_(e)
};

vjs.SeekTip.prototype.mousetime_ = function (e) {
	var minutes, seconds, seekBar, timeInSeconds;
	seekBar = this.player_.controlBar.progressControl.seekBar;
	timeInSeconds = seekBar.calculateDistance(e) * seekBar.player_.duration();
...
vjs.Html5.prototype.createEl = function () {
    ...
    if (!el || this['movingMediaElementInDOM'] === false) {
        ...
        if (el) {
            ...
            // determine if native controls should be used
            attributes = zplayer.util.mergeOptions({}, player.tagAttributes);
            if (!vjs.TOUCH_ENABLED || player.options()['nativeControlsForTouch'] !== true) {
...
//vjs.Html5['nativeSourceHandler']['canHandleSourceParse'] = function (source) {
//    var protocol = source.src.split(':').shift().toUpperCase();
//    var ext = source.src.split('.').pop().toUpperCase();
//    var type = protocol + '/' + ext;
//    return {src:source, type:type};
//};
...
vjs.Html5['nativeSourceHandler']['handleSource'] = function (source, tech) {
    console.log('html5 handleSource', source);
    ...
    init: function(player, options, ready){
        ...
        // Merge default flashvars with ones passed in to init
            flashVars = vjs.obj.merge({
                ...
                'readyFunction': 'zplayer.Flash.onReady',
                'eventProxyFunction': 'zplayer.Flash.onEvent',
                'errorEventProxyFunction': 'zplayer.Flash.onError',
...
vjs.Flash.streamToParts = function (src) {
	...
	if (connEnd !== -1) {
		...
		//chali5124@gmail.com
		// if src have a subfolder it is not working
		var streamEnd = src.lastIndexOf(':');
		var srcNew = src.substring(0, streamEnd);
		connEnd = streamBegin = srcNew.lastIndexOf('/') + 1;
    ...
    vjs.CaptionSettingsMenuItem = vjs.TextTrackMenuItem.extend({
        init: function (player, options) {
            options['track'] = {
                'kind': options['kind'],
                'player': player,
                'label': player.localize('Settings'),
                //'label': options['kind'] + ' ' + this.localize('settings'),
                'default': false,
                mode: 'disabled'
            };
...
(function () {
	...
	vjs.TextTrackSettings.prototype.createEl = function () {
		return vjs.Component.prototype.createEl.call(this, 'div', {
			...
			innerHTML: captionOptionsMenuTemplate(this)
		});
	};
	...
	function captionOptionsMenuTemplate(obj) {
		return '<div class="vjs-tracksettings">' +
			'<div class="vjs-tracksettings-colors">' +
			'<div class="vjs-fg-color vjs-tracksetting">' +
			'<label class="vjs-label">'+obj.localize('Foreground')+'</label>' +
			'<select>' +
			'<option value="">---</option>' +
			'<option value="#FFF">'+obj.localize('White')+'</option>' +
			'<option value="#000">'+obj.localize('Black')+'</option>' +
			'<option value="#F00">'+obj.localize('Red')+'</option>' +
			'<option value="#0F0">'+obj.localize('Green')+'</option>' +
			'<option value="#00F">'+obj.localize('Blue')+'</option>' +
			'<option value="#FF0">'+obj.localize('Yellow')+'</option>' +
			'<option value="#F0F">'+obj.localize('Magenta')+'</option>' +
			'<option value="#0FF">'+obj.localize('Cyan')+'</option>' +
			'</select>' +
			'<span class="vjs-text-opacity vjs-opacity">' +
			'<select>' +
			'<option value="">---</option>' +
			'<option value="1">'+obj.localize('Opaque')+'</option>' +
			'<option value="0.5">'+obj.localize('Semi-Opaque')+'</option>' +
			'</select>' +
			'</span>' +
			'</div>' + // vjs-fg-color
			'<div class="vjs-bg-color vjs-tracksetting">' +
			'<label class="vjs-label">'+obj.localize('Background')+'</label>' +
			'<select>' +
			'<option value="">---</option>' +
			'<option value="#FFF">'+obj.localize('White')+'</option>' +
			'<option value="#000">'+obj.localize('Black')+'</option>' +
			...
			'<option value="">---</option>' +
			'<option value="1">'+obj.localize('Opaque')+'</option>' +
			'<option value="0.5">'+obj.localize('Semi-Transparent')+'</option>' +
			'<option value="0">'+obj.localize('Transparent')+'</option>' +
			'</select>' +
			'</span>' +
			'</div>' + // vjs-bg-color
			'<div class="window-color vjs-tracksetting">' +
			'<label class="vjs-label">'+obj.localize('Window')+'</label>' +
			'<select>' +
			'<option value="">---</option>' +
			'<option value="#FFF">'+obj.localize('White')+'</option>' +
			'<option value="#000">'+obj.localize('Black')+'</option>' +
			...
			'<label class="vjs-label">'+obj.localize('Font Size')+'</label>' +
			'<select>' +
			...
			'<label class="vjs-label">'+obj.localize('Text Edge Style')+'</label>' +
			'<select>' +
			'<option value="none">'+obj.localize('None')+'</option>' +
			'<option value="raised">'+obj.localize('Raised')+'</option>' +
			'<option value="depressed">'+obj.localize('Depressed')+'</option>' +
			'<option value="uniform">'+obj.localize('Uniform')+'</option>' +
			'<option value="dropshadow">'+obj.localize('Dropshadow')+'</option>' +
			'</select>' +
			'</div>' + // vjs-edge-style
			'<div class="vjs-font-family vjs-tracksetting">' +
			'<label class="vjs-label">'+obj.localize('Font Family')+'</label>' +
			'<select>' +
			'<option value="">'+obj.localize('Default')+'</option>' +
			'<option value="monospaceSerif">'+obj.localize('Monospace Serif')+'</option>' +
			'<option value="proportionalSerif">'+obj.localize('Proportional Serif')+'</option>' +
			'<option value="monospaceSansSerif">'+obj.localize('Monospace Sans-Serif')+'</option>' +
			'<option value="proportionalSansSerif">'+obj.localize('Proportional Sans-Serif')+'</option>' +
			'<option value="casual">'+obj.localize('Casual')+'</option>' +
			'<option value="script">'+obj.localize('Script')+'</option>' +
			'<option value="small-caps">'+obj.localize('Small Caps')+'</option>' +
			'</select>' +
			'</div>' + // vjs-font-family
			'</div>' +
			'</div>' +
			'<div class="vjs-tracksettings-controls">' +
			'<button class="vjs-default-button">'+obj.localize('Defaults')+'</button>' +
			'<button class="vjs-done-button">'+obj.localize('Done')+'</button>' +
			'</div>';
	}
